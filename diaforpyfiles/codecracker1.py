...
def elexosS(g):#xwris attempts
    while len(g) !=4:#error gia ta len
...
def elexos(g):
    while len(g) !=4:#elegxos me attempts na epanalamvaontai kai na upenthimizontai
        print('The secret code has exactly four colors. Try again! ')
        print('ATTEMPT' , o , )
        return False#gia thn loopa while akolouthei elegxos me isdigit ka elegxos aritmwn
    while  int(g[0]) > 6 or not g.isdigit() or int(g[1])> 6 or int(g[2])>6 or int(g[3])>6 or int(g[0]) <1 or int(g[1])<1 or int(g[2])<1 or int(g[3])< 1:
        print('You can only use 1, 2, 3, 4, 5, 6 as colors. Try again!')
        print('ATTEMPT', o ,)
        return False
...
#prepei na 4axnei to megalutero len meta apo ta set wste na na ta elex3ei ola
